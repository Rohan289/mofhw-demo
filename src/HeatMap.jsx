...
function HeatMap({covidData}) {
  const [tooltipContent, setTooltipContent] = useState('');
  const [data, setData] = useState(getHeatMapData());

  useEffect(() => {
      console.log("Covid data map",covidData);
    let temp = data;
                temp.map((heatMapElement) => {
                    covidData.map((covidDataElement) => {
                        if(covidDataElement.state_name === heatMapElement.state) {
                            heatMapElement.value = covidDataElement.new_active
                        }
                    })
                })
  ...
  return (
    <div className="full-width-height container">
      <h1 className="no-margin center">Active Covid Cases</h1>
